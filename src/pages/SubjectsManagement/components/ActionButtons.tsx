...
const ActionButtons = memo(({ record, onView, onEdit, onDelete }: ActionButtonsProps) => (
  <>
    <Access permission={ALL_PERMISSIONS.SUBJECTS?.GET_BY_ID} hideChildren={true}>
      <Button
        title="Xem chi tiết"
        icon={<EyeOutlined />}
    ...
    <Access permission={ALL_PERMISSIONS.SUBJECTS?.UPDATE} hideChildren={true}>
      <Button
        title="Chỉnh sửa"
        icon={<EditOutlined />}
        type="primary"
    ...
    <Access permission={ALL_PERMISSIONS.SUBJECTS?.DELETE} hideChildren={true}>
      <Popconfirm
